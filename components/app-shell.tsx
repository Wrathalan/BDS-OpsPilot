  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${mobileOpen ? "mobile-open" : ""}`}>
        ...
        <div className="tenant-switch"><span className="tenant-avatar">O</span><span><small>Tenant</small><strong>OpsPilot Live</strong></span><ChevronDown size={15} /></div>
        ...
        <div className="sidebar-foot"><div className="sim-status"><span className="pulse-ring"><i /></span><span><strong>Agent gateway</strong><small>Live enrollment enabled</small></span></div><button onClick={logout} className="profile-button"><span className="user-avatar">{user.name.split(" ").map((part) => part[0]).join("")}</span><span><strong>{user.name}</strong><small>{user.roleName}</small></span><LogOut size={16} /></button></div>
      </aside>
      ...
      <div className="app-main">
        <header className="topbar">
          ...
          <div className="topbar-actions"><span className="sim-badge"><span /> LIVE</span><button className="icon-button" onClick={toggleTheme} aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} theme`}>{theme === "dark" ? <Sun size={18} /> : <Moon size={18} />}</button><div className="notification-wrap"><button className="icon-button" onClick={() => setNotifications(!notifications)} aria-label="Notifications"><Bell size={18} /><i /></button>{notifications && <div className="notification-popover"><strong>Notifications</strong><button onClick={() => router.push("/alerts")}>Live agent alerts <small>Open alert center</small></button><button onClick={() => router.push("/devices")}>Endpoint check-ins <small>View enrolled devices</small></button><button onClick={() => router.push("/administration")}>Enrollment tokens <small>Manage agent access</small></button></div>}</div></div>
        </header>
