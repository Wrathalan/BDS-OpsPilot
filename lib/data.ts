...
export async function getConsoleData(user: SessionUser) {
  ...
  const [organizations, devices, alerts, patches, automations, automationRuns, tickets, policies, users, auditEvents, reports, notifications, metrics, enrollmentTokens, technicianInvites] = await Promise.all([
    ...
    db.user.findMany({ where: { tenantId: user.tenantId, deletedAt: null }, select: { id: true, email: true, username: true, name: true, active: true, allOrganizations: true, lastLoginAt: true, createdAt: true, role: { select: { id: true, name: true, description: true, systemKey: true } }, scopes: { select: { organizationId: true } } }, orderBy: { name: "asc" } }).then((accounts) => accounts.map((account) => ({ ...account, protectedAccount: account.username.toLowerCase() === (process.env.BOOTSTRAP_ADMIN_USERNAME || "root").toLowerCase() }))),
